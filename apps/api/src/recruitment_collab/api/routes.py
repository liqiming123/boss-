...
# --- Compatibility with extensions installed before per-row reconciliation --
#
# Scanning is per-row now, so these anchors filter nothing: no candidate is
# skipped because of them. They are kept because an installed extension still
# reads and writes them during its own sweep, and the 404 it got after they were
# removed aborted that sweep — those machines stopped syncing entirely until
# someone updated them. The original contract is preserved verbatim so every
# installed version keeps working, and the values still feed the operations
# "最近同步" reading.
HISTORICAL_RESCAN_WATERMARK = datetime(1970, 1, 1, tzinfo=timezone.utc)


def _scan_checkpoint_row(db: Session, company_id: str, platform: str, account_display_name: str):
    return db.scalar(
        select(ConversationScanCheckpoint).where(
            ConversationScanCheckpoint.company_id == company_id,
...
def get_scan_checkpoint(
    ...
    company_id = plugin_company_for_account(actor, account_display_name, db)
    row = _scan_checkpoint_row(db, company_id, platform, account_display_name)
    if not row:
        return {
...
def restart_scan(
    ...
    row = _scan_checkpoint_row(db, company_id, platform, account_name)
    # Keep an explicit old anchor instead of deleting the row: a missing row is
    # read as "now" by the GET above, which would make every existing
    # conversation look older and silently skip the requested rescan.
    scan_id = secrets.token_urlsafe(16)
    marker = {"mode": "HISTORICAL_RESCAN", "scan_id": scan_id, "complete": False}
    if row:
        row.completed_through_at = HISTORICAL_RESCAN_WATERMARK
        row.cursor_json = marker
    else:
        db.add(
            ConversationScanCheckpoint(
                company_id=company_id,
...
def put_scan_checkpoint(body: ScanCheckpointRequest, actor: Actor = Depends(plugin_actor), db: Session = Depends(get_db)) -> dict[str, Any]:
    ...
    row = _scan_checkpoint_row(db, company_id, body.platform, body.account_display_name)
    if body.cursor.get("complete") is not True:
        # Older extensions reported the newest individual candidate as if an
        # entire traversal had finished. Never let a partial report move the
        # high-water mark past candidates that were not seen.
    ...
    if row and (row.cursor_json or {}).get("mode") == "HISTORICAL_RESCAN":
        ...
        # requested a rescan; only the tab carrying this rescan's server-issued
        # id may complete it.
        expected_scan_id = (row.cursor_json or {}).get("scan_id")
        if not expected_scan_id or body.cursor.get("scan_id") != expected_scan_id:
            return {"completed_through_at": _aware_utc(row.completed_through_at).isoformat(), "cursor": row.cursor_json or {}, "accepted": False}
    if row:
        stored = _aware_utc(row.completed_through_at)
        incoming = _aware_utc(body.completed_through_at)
        if incoming > stored:
