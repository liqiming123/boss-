...
def upgrade() -> None:
    ...
    bind.execute(
        sa.text(
            """
            UPDATE recruitment_recruiters bound
            SET role = 'ADMIN', updated_at = CURRENT_TIMESTAMP
            WHERE bound.display_name = '李启明'
              AND bound.feishu_display_name = '李启明'
              AND bound.feishu_open_id IS NOT NULL
              AND bound.role NOT IN ('ADMIN', 'HR_MANAGER')
              AND EXISTS (
                  SELECT 1
                  FROM recruitment_recruiters legacy_admin
                  WHERE legacy_admin.company_id = bound.company_id
