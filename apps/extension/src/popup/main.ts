...
<details id="more" hidden><summary>更多设置</summary>
  ...
  <button id="restart" class="secondary">重新扫描历史记录</button>
...
async function load() { try { const a = await getAuth(); if(a.pendingFeishuLogin && !(await poll(a.pendingFeishuLogin))) return; const current=await getAuth(); if (!current.accessToken) { account.textContent="尚未连接招聘账号"; status.className="muted"; status.textContent="绑定飞书后即可同步和接收提醒"; showBoundUi(false); return; } const me=await apiRequest<{display_name:string;role:string}>("/plugin/me"); account.textContent=`当前招聘人：${me.display_name}`; status.className="ok"; status.textContent="● 已连接，自动同步已开启"; showBoundUi(true); const isAdmin=me.role.toUpperCase()==="ADMIN"; dailyCard.hidden=!isAdmin; root.querySelector<HTMLButtonElement>("#dailyTimerTest")!.hidden=!isAdmin; if (isAdmin) await showDailyStatus(); } catch (error) { const current=await getAuth(); if(!current.accessToken){account.textContent="尚未连接招聘账号";status.className="muted";status.textContent="绑定飞书后即可同步和接收提醒";showBoundUi(false);return;} status.className="error"; status.textContent=error instanceof Error?error.message:"连接状态读取失败"; showBoundUi(true); dailyCard.hidden=true; } }
...
restart.onclick=async()=>{restart.disabled=true;try{const tab=await currentBossTab();if(!tab?.id)throw new Error("请先打开 BOSS 沟通页");const accountDisplayName=await currentBossAccount();if(!accountDisplayName)throw new Error("请先打开 BOSS 沟通页");const query=new URLSearchParams({account_display_name:accountDisplayName,platform:"boss"});await apiRequest(`/plugin/conversations/restart?${query.toString()}`,{method:"POST"});status.className="ok";status.textContent="已重置历史扫描锚点，请刷新 BOSS 页面开始历史扫描";await chrome.tabs.reload(tab.id);}catch(error){status.className="error";status.textContent=error instanceof Error?error.message:"重新同步失败"}finally{restart.disabled=false;}};
