...
function text(selector: string) {
  return (
    document.querySelector<HTMLElement>(selector)?.dataset.value?.trim() ??
    document.querySelector<HTMLElement>(selector)?.textContent?.trim() ??
    ""
  );
}
export class MockRecruitmentAdapter implements RecruitmentSiteAdapter {
  readonly platform = "mock";
  canHandle(url: string) {
    return (
      new URL(url).hostname === "localhost" && new URL(url).port === "5174"
    );
  }
  isCandidateConversationPage() {
    return this.canHandle(location.href);
  }
  async extractAccount(): Promise<Extraction<AccountData>> {
    const value = text("[data-recruitment-account]");
    return value
      ? { status: "OK", value: { displayName: value } }
      : { status: "ERROR", errorCode: "ACCOUNT_MISSING" };
  }
  async extractCandidate(): Promise<Extraction<CandidateData>> {
    const value = text("[data-candidate-name]");
    return value
      ? {
          status: "OK",
          value: {
            displayName: value,
            platformCandidateId: document.querySelector<HTMLElement>(
              "[data-candidate-id]",
            )?.dataset.candidateId,
          },
        }
      : { status: "ERROR", errorCode: "CANDIDATE_MISSING" };
  }
  async extractJob(): Promise<Extraction<JobData>> {
    const value = text("[data-job-name]");
    return value
      ? { status: "OK", value: { displayName: value } }
      : { status: "ERROR", errorCode: "JOB_MISSING" };
  }
  ...
  async getDiagnostics(): Promise<AdapterDiagnostics> {
    ...
    const errors = [a, c, j]
      ...
      .map((v) => (v.status === "ERROR" ? v.errorCode : ""));
    return {
      ...
      jobStatus: j.status,
      platformIdStatus:
        c.status === "OK" && c.value.platformCandidateId ? "OK" : "MISSING",
      errorCodes: errors,
      sanitizedContext: {
        path: location.pathname,
        fieldCount: 3 - errors.length,
      },
    };
  }
}
