...
飞书是候选人业务数据的长期权威来源。数据库长期保留设备身份、哈希键、飞书记录 ID、幂等状态、水位和必要审计，不保存聊天正文或图片字节。`data_retention_worker` 默认每 30 天执行一次可配置清理（`RETENTION_RUN_INTERVAL_DAYS`）；同步成功的候选人 Outbox 和通知 Outbox 会立即清空载荷。候选人来源在没有活动同步任务且超过缓存期后最小化，即使从未拿到飞书记录也不会无限期保留完整字段；活动 `PENDING/PROCESSING` 任务会暂时保护未同步来源。清理 Worker 在两次清理之间只保留心跳，不重复扫描数据库。
