...
4. 点击候选人即实时同步：表格里没有就建行，会话时间变新就刷新“更新时间”；只有用户实际发送消息后，后端才登记业务跟进和 `MESSAGE_SENT` 事件，并通过事务 Outbox 发送提醒。
5. 用户授权后，系统可以同步聊天区域截图和简历附件到受限飞书多维表；API 不落盘图片字节、Cookie、密码或完整 HTML。

扩展兼容 Chrome 和 Microsoft Edge，使用 Chromium MV3。定时历史补扫**每天 23:30（北京时间）**跑一轮，或点弹窗"立即检查"：范围是**当天周期（23:30→次日 23:30）内有更新**的会话，逐个打开、同步记录并采集聊天区域长图；如果 23:30 时浏览器没开，下次打开沟通页会在空闲时自动补一轮，范围相同。消息发送（包括面试邀约）不截图，聊天长图只由补扫采集。详细流程见 [`docs/BOSS_FEISHU_FLOW.md`](docs/BOSS_FEISHU_FLOW.md)。
